...
class BiometricModel:
    ...
    def actuarialtable2(self, cohort1, cohorte2):
        ...
        eys = pd.Series(e2)
        at1, at2 = 0, 0
        if int(cohort1[-1:]) != 0 or int(cohorte2[-1:]) != 0:
            at1 = cohort1.index[-1] + 1
...
class MultipleLives:
    ...
    # def lifeplot2(self, cohort1, cohort2, model=None, dx_log: bool = False,
    #               interactive: bool = False, death: bool = False):
    #     """
    #     :param dx_log:
    #     :param model:
    ...
    #             plt.ylabel('LogDeath ($d_{x}$) and ($d_{y}$)', fontweight='bold')
    #             plt.plot(np.log(qx1), label=f'{cohort1.name}', color='darkorange')
    #             plt.plot(np.log(qx2), label=f'{cohort2.name}', color='midnightblue')
    #         else:
    #             plt.plot(dx1, label=f'{cohort1.name}', color='darkorange')
    ...
    #         import plotly.graph_objects as go
    #         if not death:
    #             plot1 = go.Scatter(x=cohort1.index, y=cohort1, mode='lines',
    #                                name=f'{cohort1.name}', line=dict(color='midnightblue'))
    #             plot2 = go.Scatter(x=cohort2.index, y=cohort2, mode='lines',
    #                                name=f'{cohort2.name}', line=dict(color='darkorange'))
    #             if model is not None:
    #                 plot3 = go.Scatter(x=model.index, y=model, mode='lines',
    #                                    name=f'{model.name}', line=dict(color='darkred'))
    #                 figure = go.Figure([plot1, plot2, plot3])
    #                 figure.update_layout(title_text=f'Survival Plot of {cohort1.name}'
    #                                                 f', {cohort2.name} and {model.name}',
    #                                      xaxis_title='Age',
    #                                      yaxis_title='Living people ($l_{x}$), ($l_{y}$) and ($l_{z}$)',
    #                                      font_family='COLLEGE', font_color='black',
    #                                      title_font_color='red', title_font_family='New Times Rowan',
    #                                      legend_title_font_color='green')
    #                 figure.show()
    #
    #             figure = go.Figure([plot1, plot2])
    #             figure.update_layout(title_text=f'Survival Plot of {cohort1.name}'
    #                                             f' and {cohort2.name}',
    #                                  xaxis_title='Age',
    #                                  yaxis_title='Living people ($l_{x}$) and ($l_{y}$)',
    #                                  font_family='COLLEGE', font_color='black',
    #                                  title_font_color='red', title_font_family='New Times Rowan',
    #                                  legend_title_font_color='green')
    #
    ...
    #         else:
    #
    #             plot1 = go.Scatter(x=cohort1.index, y=dx1, mode='lines',
    #                                name=f'{cohort1.name}', line=dict(color='midnightblue'))
    #
    #             plot2 = go.Scatter(x=cohort2.index, y=dx2, mode='lines',
    #                                name=f' {cohort2.name}', line=dict(color='darkorange'))
    #             if model is not None:
    #                 pxm = ((model / model.shift(1)).shift(-1)).fillna(0)
    #                 qxm = 1 - pxm
    #                 dxm = qxm * model
    #                 plot3 = go.Scatter(x=model.index, y=dxm, mode='lines',
    #                                    name=f'{model.name}', line=dict(color='darkred'))
    #                 figure = go.Figure([plot1, plot2, plot3])
    #                 figure.update_layout(title_text=f'Death Plot of {cohort1.name} '
    #                                                 f', {cohort2.name} and {model.name}',
    ...
    #
    #             figure = go.Figure([plot1, plot2])
    #             figure.update_layout(title_text=f'Death Plot of {cohort1.name} and {cohort2.name}',
    #                                  xaxis_title='Age',
    #                                  yaxis_title='Death of people $d_{x}$',
    #                                  font_family='COLLEGE', font_color='black',
    #                                  title_font_color='red', title_font_family='New Times Rowan',
    ...
    #             figure.show()
    #
    # def lifeplots(self, cohort1, cohort2, model=None, interactive: bool = False,
    #               death: bool = False, qx_log: bool = False):
    #     """
    #     :param qx_log:
    #     :param model:
    ...
    #                     plt.plot(np.log(qx2), label=f'{cohort2.name}', color='midnightblue')
    #                     if model is not None:
    #                         plt.plot(np.log(qxm), label=f'{model.name}', color='red')
    #         plt.legend()
    #         plt.show()
    #
    #     else:
    ...
    def dissolution(self, c1, c2, age_x: int, age_y: int,
                    t: int = 1):
        """
        This is the probability that at least one individual will die within t years.
        ...
        :return: The probability that at least one individual will die within t years
        ...
        probabilityx = np.roll(c1[sumx], 1) / c1[age_x]
        probabilityy = np.roll(c2[sumy], 1) / c2[age_y]
        proba = probabilityx * probabilityy
        probability = 1 - proba
        return probability
...
class SpainOperation:
    ...
    # def renta_supervivencia(self, tabla_mortalidad):  ## Ojo a usar PERM para Rentas y PASEM seguro
    #     datatable = tabla_mortalidad
    #     birth_year = int(input('Ingrese el año de nacimiento del asegurado: '))
    ...
    #     jj = (jon + jon1).flatten().round(4)  # Tabla creada con fraccionamiento
    #     t = np.arange((m * frac_freq), ((m + n) * frac_freq))  # t Periodos del sumatorio
    #
    #     a1 = input('Anticipada o Vencida: ').lower()
    #     interest = float(input('Tasa de interés: '))
    #     if a1 in ('anticipada', 'prepagable'):
    #         v = (1 + interest) ** -(t / frac_freq)
    #         p = jj[x * frac_freq + t] / jj[x * frac_freq]  # px supervivencia prepagable
    #         capital = input('Ingrese el tipo de Renta:\n'
    #                         '"Constante"\n'
    #                         '"Aritmetica" o "Lineal"\n'
    #                         '"Geometrica" o "Acumulativa"\n').lower()  # Constante, aritmetica, geometrica
    #         if capital == 'constante':
    #             k1 = float(input('Cuantìa del capital: '))
    #         elif capital in ('aritmetica', 'lineal'):
    #             k2 = float(input('Primer termino: '))
    #             k2_ = float(input('Segundo termino: '))
    #             k1 = k2 + k2_ * ((t / frac_freq) - (m * frac_freq))
    ...
    #         else:
    #             print('Error! Solo se permite Constante, Aritmetica o Geometrica')
    #
    #     elif a1 in ('vencida', 'pospagable'):
    #         v = (1 + interest) ** -((t + 1) / frac_freq)
    #         p = jj[x * frac_freq + t + 1] / jj[x * frac_freq]  # px supervivencia pospagable
    #         capital = input('Ingrese el tipo de Renta: ').lower()
    #         if capital == 'constante':
    #             k1 = float(input('Cuantìa del capital: '))
    #         elif capital in ('aritmetica', 'lineal'):
    #             k2 = float(input('Primer termino: '))
    ...
    #             k1 = k3 * k33 ** (t - m * frac_freq)
    #     else:
    #         print('Opciòn no valida. Se permite Anticipada, Vencida, Prepagable o Pospagable')
    #     valor = sum(k1 * p * v)
    #     print('El Valor de la renta es: ', round(valor, 2))  ## Quitar de la version oficial
    #
    # def renta_supervivencia2(self, tabla_mortalidad):
    #     datatable = tabla_mortalidad
    #     birth_year = int(input('Ingrese el año de nacimiento del asegurado: '))
    #     frac_freq = int(input('Frecuencia deseada de fraccionamiento: '))
    #     x = int(input('Ingrese la edad del tomador del seguro: '))
    ...
    #         v = (1 + interest) ** -(t / frac_freq)
    #         p = jj[x * frac_freq + t] / jj[x * frac_freq]  # px supervivencia prepagable
    #         capital = input('Ingrese el tipo de Renta: ').lower()  # Constante, aritmetica, geometrica
    #         if capital == 'constante':
    #             k1 = float(input('Cuantìa del capital: '))
    #         elif capital in ('aritmetica', 'lineal'):
    #             k2 = float(input('Primer termino: '))
    ...
    #         print('Opciòn no valida. Se permite Anticipada, Vencida, Prepagable o Pospagable')
    #     valor = sum(k1 * p * v)
    #     print('El Valor de la renta es: ', round(valor, 2))
    ...
    def seguro_fallecimiento(self, tabla_mortalidad):
        ...
        if a2 in 'no':
            ...
            a3 = int(input('Ingrese la frecuencia de variaciòn de pago: '))
            if capital == 'constante':
                k1 = int(input('Cuantìa del capital: '))
            elif capital in ('aritmetica', 'lineal'):
                k2 = float(input('Primer termino: '))
...
class InsuranceBenefits:
    ...
    def increasing_lifeinsurance(self, cohort, age, n=0, m=0, i=.02, kind='whole'):
        ...
        :param kind: The kind of decreasing annuity. "Whole" or "Temporary" are accepted.
    ...
    def temporary_annuity(self, cohort, age: int, n: int = 'whole', m=12,
                          i: float = .02, capital: float = 1, payments: str = 'advance'):  # axn en lifecontingencies
        ...
        Por defecto calcula la annuity-advance, puede pasarsele la arrears.
        ...
        :param m: Number of fraccional payments per period. By default 1.
    ...
    def deferred_annuity(self, cohort, age: int, n: int = 10,
        ...
        if payments == 'arrears':
            ...
            print('Please the payments should be specified as "due", "arrears" or "whole" either')
    ...
    def endowment(self, cohort, age: int, time: int = 1, i: float = .02,
        ...
        :param time: The length of the contract.
    ...
    def increasing_annuity(self, cohort, x, m=0, n=1, interest=.02):
        ...
        if n != omega1:
            ...
            print('Error! Type a new "n" ')
        px1 = (cohort.shift(-1) / cohort).dropna()
        t = np.arange(m, (m + n))
        v = (1 + interest) ** - t
        u = t+1
        px = px1[x+t]/px1[x]
...
# file = r'C:\Users\Josè Valencia\Desktop\Barcelona Master\Semestre ll\Matematica del Seguro\SpainTables.csv'
# data = pd.read_csv(file)
# f1 = r'C:\Users\Josè Valencia\Desktop\Barcelona Master\Semestre ll\Matematica del Seguro\PER2020_Col_1er.orden.csv'
# data2 = pd.read_csv(f1)

# print(SpainOperation().renta_supervivencia2(data2))
# print(SpainOperation().constant_annuity(datatable=data, x=55, m=10, n=20,
#                                        capital=500, h=12)
#

# def renta_supervivencia(tabla_mortalidad):
#     datatable = tabla_mortalidad
#     birth_year = int(input('Ingrese el año de nacimiento del asegurado: '))
#     frac_freq = int(input('Frecuencia deseada de fraccionamiento: '))
#     x = int(input('Ingrese la edad del tomador del seguro: '))
...
# print(SpainOperation().renta_constante(data2, x=35, birth_year=1984, m=30, capital=4000,
#                                         h=1, interest=.025,  pospagable='Si'))
